...
import { TASK_PRIORITIES, TASK_STATUSES, TASK_STATUS, TASK_TYPES } from '@/constants/task';
...
export function TaskForm({ open, onOpenChange, task }: Props) {
  ...
  const body = (
    <form onSubmit={handleSubmit} onKeyDown={handleKey} className="flex min-h-0 flex-1 flex-col">
      <div className="flex flex-col gap-4 overflow-y-auto">
        ...
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
          ...
          <div className="flex flex-col gap-1.5">
            ...
            <Select
              value={form.type}
              onValueChange={(v) => setForm((p) => ({ ...p, type: v as TaskType }))}
            >
              <SelectTrigger>
                <SelectValue />
              </SelectTrigger>
      ...
      <div className="mt-5 flex flex-col-reverse gap-2 border-t border-border pt-4 sm:flex-row sm:items-center sm:justify-between">
        {isEdit ? (
          <Button
            type="button"
            variant="ghost"
            className="text-priority-high hover:bg-priority-high-bg hover:text-priority-high sm:w-auto"
        ...
        <div className="flex gap-2 sm:justify-end">
          ...
          <Button type="submit" className="flex-1 sm:flex-initial" disabled={!titleValid}>
            {t.form.save}
            <span className="hidden text-xs opacity-70 sm:inline">· {t.form.saveHint}</span>
          </Button>
        </div>
      </div>
      ...
      {showDeleteConfirm && (
        <div
          ...
          className="absolute inset-0 z-10 flex items-center justify-center rounded-lg bg-surface/95 p-6 backdrop-blur-sm"
        >
          <div className="w-full max-w-sm text-center">
            <h3 id="confirm-delete-title" className="font-fraunces text-xl italic text-ink">
              {t.form.confirmDeleteTitle}
            </h3>
            <p className="mt-2 text-sm text-muted-foreground">
              {t.form.confirmDeleteBody}
            </p>
    ...
    return (
      <Sheet open={open} onOpenChange={onOpenChange}>
        <SheetContent className="relative h-[92vh]">
          <SheetHeader>
            <SheetTitle className="font-fraunces text-xl italic">
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="relative max-h-[90vh] overflow-hidden">
        <DialogHeader>
          <DialogTitle className="font-fraunces text-xl italic">
            {isEdit ? t.form.editTitle : t.form.addTitle}
          </DialogTitle>
